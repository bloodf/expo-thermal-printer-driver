...
import { text, qr, feed, cut, line, raw, columns } from '../../../document/nodes';
...
describe('ESCPOSCompiler', () => {
  ...
  it('compiles bold text', () => {
    const bytes = compileDocument([text('Bold', { bold: true })], { paperWidthMm: 58 });
  ...
  it('compiles centered text', () => {
    const bytes = compileDocument([text('Center', { align: 'center' })], { paperWidthMm: 58 });
    const alignIdx = findSubarray(bytes, CMD.ALIGN_CENTER);
    expect(alignIdx).toBeGreaterThanOrEqual(0);
  });

  it('compiles QR code', () => {
    const bytes = compileDocument([qr('https://test.com', { size: 5 })], { paperWidthMm: 58 });
  ...
  it('compiles partial cut', () => {
    const bytes = compileDocument([cut({ partial: true })], { paperWidthMm: 58 });
