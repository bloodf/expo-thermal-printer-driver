...
const BARCODE_FORMAT_MAP: Record<
  Extract<Node, { type: 'barcode' }>['format'],
  number
> = {
  UPC_A: 65,
  UPC_E: 66,
...
const HRI_MAP: Record<
  NonNullable<Extract<Node, { type: 'barcode' }>['hri']>,
  number
> = {
  none: 0,
  above: 1,
...
const QR_ERROR_MAP_TYPED: Record<
  NonNullable<Extract<Node, { type: 'qr' }>['errorLevel']>,
  number
> = QR_ERROR_MAP;
...
function compileNode(
  ...
  switch (node.type) {
    case 'text':
      return compileText(node, bytes, ctx);
    case 'line':
      return compileLine(node, bytes, ctx);
    case 'qr':
      return compileQR(node, bytes);
    case 'barcode':
      return compileBarcode(node, bytes);
    case 'feed':
      return compileFeed(node, bytes);
    case 'cut':
      return compileCut(node, bytes);
    case 'raw':
      return compileRaw(node, bytes);
    case 'columns':
      return compileColumns(node, bytes, ctx);
    case 'table':
      return compileTable(node, bytes, ctx);
    case 'spacer':
      return compileSpacer(node, bytes);
    case 'image':
      return; // Images are handled natively, not compiled in JS
  }
}
...
function compileText(
  ...
  if (style?.underline === true || style?.underline === 'single')
    bytes.push(...CMD.UNDERLINE_1);
...
function compileLine(
  ...
  const repeated = char
    .repeat(Math.ceil(ctx.charsPerLine / char.length))
    .slice(0, ctx.charsPerLine);
...
function compileQR(node: Extract<Node, { type: 'qr' }>, bytes: number[]): void {
  ...
  const errLevel =
    QR_ERROR_MAP_TYPED[node.errorLevel ?? 'M'] ?? QR_ERROR_MAP_TYPED.M;
  bytes.push(...qrCmd(node.content, size, errLevel));
  bytes.push(LF);
}

function compileBarcode(
  node: Extract<Node, { type: 'barcode' }>,
  bytes: number[]
): void {
...
function compileTable(
  ...
  const charWidths = colWidths.map((w: number) =>
    Math.floor(w * ctx.charsPerLine)
  );
  ...
  if (def.headers) {
    ...
    bytes.push(
      ...Array.from(new TextEncoder().encode(formatRow(def.headers))),
      LF
    );
    ...
    if (def.border === 'single') {
      bytes.push(
        ...Array.from(new TextEncoder().encode('-'.repeat(ctx.charsPerLine))),
        LF
      );
